...
- `artifacts/api-server/src/services/archive-operations.ts` — compatibility adapter mapping the legacy single-operation contract onto the action engine. An "archive operation" is not a parallel concept: `createArchiveOperation` calls `createActionProposal`, so the returned id *is* an action proposal id and acquisition imports are reviewable on the standard action surface.
...
- `docs/capability-surface-audit.md` grades every capability on visible → understandable → actionable → closed loop. Re-run `node scripts/audit-capability-surface.mjs` after adding endpoints: an operation with no UI entry point is unfinished product surface, and "an AI tool can call it" does not count. The count finds missing doors, not missing signposts — a capability reached through a shared surface still needs something pointing at it, so read the remaining gaps individually rather than treating the number as a score.
...
- Execution closes its own loop: `runArchivePostflight` re-scans the archive, refreshes Plex when configured, and re-runs reconciliation. The review surface reports this as WHAT HAPPENED NEXT from persisted `postflight` facts only — it never claims a Plex refresh that did not happen, and postflight errors suppress the all-clear.
