...
let baseUrl = "";
/**
 * Every stream opened by a test. A test that fails before its own close()
 * would otherwise leave an open SSE response, and the run would hang instead
 * of reporting the failure.
 */
const openStreams = new Set<{ close: () => Promise<void> }>();
...
describe("archive scan events", { concurrency: false }, () => {
  ...
  test("the SSE stream sends an initial snapshot, live events, and reconnects with fresh state", async () => {
    ...
    const subscribersBefore = archiveScanSubscriberCount();

    const first = await openScanEventsStream();
    const snapshotFrame = await first.waitForFrame((frame) => frame.event === "snapshot");
    // The very first frame must be the snapshot: a client that reconnects
    // mid-scan learns the current state without waiting for the next event.
    assert.equal(first.frames[0].event, "snapshot", "the snapshot is the first frame on the stream");
    assert.equal(
      archiveScanSubscriberCount(),
      subscribersBefore + 1,
      "an open stream registers exactly one subscription",
    );
    ...
    await first.close();
    // A disconnecting browser must release its subscription and heartbeat.
    // Without this the server accumulates a dead listener per reconnect.
    for (let attempt = 0; attempt < 200 && archiveScanSubscriberCount() > subscribersBefore; attempt += 1) {
      await new Promise((resolve) => setTimeout(resolve, 10));
    }
    assert.equal(
      archiveScanSubscriberCount(),
      subscribersBefore,
      "closing the stream unsubscribes the client",
    );
    ...
    assert.equal(
      archiveScanSubscriberCount(),
      subscribersBefore,
      "the reconnected stream also unsubscribes on close",
    );
