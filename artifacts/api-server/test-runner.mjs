...
// Each test file runs in its own process with its own throwaway database so
// suites stay isolated from each other's fixtures.
const suites = [
  { file: "ownership.test.ts", legacyDatabase: true },
  { file: "scan-events.test.ts", legacyDatabase: false },
];
...
for (const suite of suites) {
  ...
  const outputFile = path.join(testDir, suite.file.replace(/\.ts$/, ".mjs"));
  const databaseFile = path.join(testDir, `${suite.file.replace(/\.ts$/, "")}.sqlite`);

  try {
    if (suite.legacyDatabase) {
    ...
    await build({
      entryPoints: [path.join(artifactDir, "test", suite.file)],
      bundle: true,
      format: "esm",
      platform: "node",
      target: "node22",
...
`,
    ...
    failed = true;
    process.exitCode = 1;
    console.error(`Test suite ${suite.file} could not be prepared:`, error);
  } finally {
    await rm(testDir, { recursive: true, force: true });
  }
}

if (!failed) console.log(`All ${suites.length} test suites passed.`);
