...
/**
 * Path-based identity constructor for callers that are not in the middle of a
 * scan (the safe mutation engine links moved files back to their identity).
 */
...
function scanRoots(settings: SettingsRecord) {
  ...
  // and naming intelligence. Nested or overlapping roots are skipped to avoid
  // double-walking the same tree: a staging directory inside an archive volume
  // is inventoried as part of that volume's walk.
  const alreadyCovered = roots.some(
    (root) => isArchivePathWithin(expanded, root) || isArchivePathWithin(root, expanded),
  );
  if (!alreadyCovered && !roots.includes(expanded) && existsSync(expanded)) roots.push(expanded);
  return roots;
}
...
async function inspectFile(filePath: string, root: string, existing: FileRow | undefined, settings: SettingsRecord, archiveScanRoots: string[]) {
  ...
  const unchanged = existing
    ...
    // Rows written before derived metadata (filename, identity link) existed
    // are not "unchanged": only a full re-inspection repairs them. A complete
    // row that merely lacks byte evidence still takes the hash-only backfill.
    && existing.filename;
  if (unchanged) {
