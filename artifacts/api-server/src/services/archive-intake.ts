...
/**
 * The archive destination an acquisition plan prepared for a job, when the job
 * was created by executing one. Plan execution deliberately does not hand the
 * final destination to the download engine: the engine's own verified move
 * lands the file at the volume root, and the plan's destination is what intake
 * promotes it to through the journal. Nothing reaches its final archive path
 * except via a journaled operation.
 */
function findPlanItemForJob(ownerId: string, jobId: number): PlanItemRow | null {
  const row = archiveDb
    .prepare(
      `SELECT id, plan_id, identity_key, title, destination_directory, final_filename, state
         FROM acquisition_plan_item
        WHERE owner_id = ? AND download_job_id = ?
        LIMIT 1`,
    )
    .get(ownerId, jobId) as PlanItemRow | undefined;
  return row ?? null;
}
...
export async function readIntakeItems(ownerId: string, settings: SettingsRecord = readSettings()) {
  ...
  const items: IntakeItem[] = jobs.map((job): IntakeItem => {
    ...
    // Intake promotes the file to the archive destination that was prepared for
    // it. For a plan-driven job that is the acquisition plan's destination (the
    // plan computed it; the download engine only landed the verified file at the
    // volume root). For any other job it is the destination the job itself was
    ...
    const planItem = findPlanItemForJob(ownerId, job.id);
    const targetPath = planItem?.destination_directory && planItem.final_filename
      ? join(planItem.destination_directory, planItem.final_filename)
      : job.destinationDirectory
        ? join(job.destinationDirectory, job.finalFilename)
        : (naming?.proposedPath ?? null);

    // A file that already sits at its prepared destination (a manual job whose
    // engine move landed it there, or a plan destination equal to the engine's
    // landing) has nothing left to promote: report it as archived, not blocked.
    const alreadyAtDestination = Boolean(stagedPath && targetPath && resolve(stagedPath) === resolve(targetPath));
    ...
    if (!stagedPath) {
      ...
      gate = {
        legal: false,
        code: "same_path",
        reason: "The verified file is already at its prepared archive destination.",
      };
    } else {
      try {
    ...
    } else if (gate.code === "same_path") {
      // The verified file already sits exactly where its prepared destination
      // says (a manual job the engine moved itself, or a plan destination that
      // matches the engine's landing). There is nothing left to promote.
      disposition = "already_in_archive";
      nextAction = "The verified file is already at its prepared archive destination; the record is part of the archive.";
    } else if (!gate.legal) {
      disposition = "blocked";
...
export async function planIntakePromotion(
  ...
  const planItem = findPlanItemForJob(ownerId, jobId);
  const operation = createOperation(
    ownerId,
    {
      ...
      proposalEvidence: {
        ...
        acquisitionPlanId: planItem?.plan_id ?? null,
        acquisitionIdentityKey: planItem?.identity_key ?? null,
      },
...
export async function applyIntakePromotion(
  ...
  // A plan-driven job has now delivered its item into the archive through the
  // journal: record the link so the plan reports the promotion (and its
  // operation) as part of its own audit trail. Rolling the operation back
  // re-opens the item on the intake queue.
  const planItem = findPlanItemForJob(ownerId, jobId);
  if (planItem) {
    archiveDb
      .prepare(
        `UPDATE acquisition_plan_item
            SET state = 'promoted', operation_id = ?, updated_at = CURRENT_TIMESTAMP
          WHERE owner_id = ? AND id = ?`,
      )
      .run(operationId, ownerId, planItem.id);
  }
