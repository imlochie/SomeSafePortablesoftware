...
const containerFailurePatterns = [
  ...
  /ebml(?:\s+(?:header|element|read|parse))?/i,
...
export function assessMediaIntegrityFailure(rawError: string): MediaIntegrityAssessment {
  if (containerFailurePatterns.some((pattern) => pattern.test(rawError))) {
    return {
