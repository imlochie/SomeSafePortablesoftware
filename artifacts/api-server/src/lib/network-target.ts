 ...
 */
export interface BinaryRequestOptions extends JsonRequestOptions {
...
export function requestBinary(options: BinaryRequestOptions): Promise<{ body: Buffer; contentType: string }> {
  ...
  return new Promise((resolve, reject) => {
    const requestOptions = {
      ...
      path: requestPath,
      method: "GET",
      headers: { Accept: allowedContentTypes.join(", "), Host: target.url.host, ...headers },
    };
    const handleResponse = (response: IncomingMessage) => {
      ...
      response.on("data", (chunk: Buffer) => {
        ...
        if (byteCount > maxResponseBytes) response.destroy(requestError(`${label} artwork exceeded the response limit.`));
        else chunks.push(chunk);
      });
      response.on("error", (error) => reject(requestError(error instanceof Error ? error.message : String(error))));
      response.on("end", () => {
        if (!response.statusCode || response.statusCode < 200 || response.statusCode >= 300) return reject(requestError(`${label} artwork request failed with HTTP ${response.statusCode ?? 0}.`));
        if (!allowedContentTypes.includes(contentType)) return reject(requestError(`${label} returned an unsupported artwork type.`));
    ...
    request.setTimeout(timeoutMs, () => request.destroy(requestError(`${label} artwork request timed out after ${timeoutMs} ms.`)));
    request.on("error", (error) => reject(requestError(error instanceof Error ? error.message : String(error))));
    request.end();
  });
}
