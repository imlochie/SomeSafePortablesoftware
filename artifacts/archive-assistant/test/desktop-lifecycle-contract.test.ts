...
describe('desktop lifecycle foundation', () => {
  it('uses the deterministic assistant overview as a Home briefing', async () => {
    ...
    expect(app).toContain('panel-home-briefing');
    expect(app).toContain('text-home-attention');
    expect(app).toContain('Nothing needs your attention.');
    expect(app).toContain('SHOW WHY AND EVIDENCE');
  });

  it('keeps major product capabilities reachable through stable user-job navigation', async () => {
    const app = await read('src/App.tsx');
    expect(app).toContain("href: '/user-portal'");
    expect(app).toContain("href: '/assistant'");
    expect(app).toContain("href: '/discover'");
    expect(app).toContain("href: '/archive'");
    expect(app).toContain("href: '/queue'");
    expect(app).toContain("href: '/history'");
    expect(app).toContain("href: '/settings'");
    expect(app).toContain('href="/archive/health"');
    expect(app).toContain('WorkloadSummary');
    expect(app).toContain('Your archive, understood');
    expect(app).toContain('Runs locally. Nothing changes without your approval.');
  });

  it('keeps workload continuity visible across queue and history', async () => {
    const app = await read('src/App.tsx');
    const workload = await read('src/components/workload-summary.tsx');
    expect(app).toContain('<WorkloadSummary compact />');
    expect(workload).toContain('panel-unified-workload');
    expect(workload).toContain('NEEDS YOU');
    expect(workload).toContain('BEING HANDLED');
    expect(workload).toContain('WAITING');
    expect(workload).toContain('BLOCKED');
    expect(workload).toContain('underlying work has not been changed');
    expect(workload).toContain('VIEW WHAT HAPPENED');
    expect(workload).toContain('I don\'t have enough information to confirm what happened.');
    expect(workload).toContain('OPEN FULL STORY');
    expect(app).toContain('workload/:workloadId');
    expect(app).toContain('What approval means:');
    expect(app).toContain('humanJobPhase');
    const detail = await read('src/pages/workload-detail.tsx');
    expect(detail).toContain('Needs your attention');
    expect(detail).toContain('Not confirmed');
    const consequence = await read('src/components/action-consequence-preview.tsx');
    expect(consequence).toContain('panel-consequence-preview');
    expect(consequence).toContain('WHAT WILL NOT HAPPEN YET');
    expect(app).toContain('humanJobPhase');
  });
