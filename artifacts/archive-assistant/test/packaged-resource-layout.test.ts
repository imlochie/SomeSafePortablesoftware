...
describe('packaged resource layout', () => {
  ...
  it('stops only the Archive Assistant process tree before NSIS overwrites the sidecar', () => {
    const hooks = readFileSync(path.join(srcTauriDir, 'windows', 'installer-hooks.nsi'), 'utf8');
    const config = JSON.parse(readFileSync(path.join(srcTauriDir, 'tauri.conf.json'), 'utf8')) as {
      bundle?: { windows?: { nsis?: { installerHooks?: string } } };
    };
    expect(config.bundle?.windows?.nsis?.installerHooks).toBe('windows/installer-hooks.nsi');
    expect(hooks).toContain('NSIS_HOOK_PREINSTALL');
    expect(hooks).toContain('archive-assistant-desktop.exe');
    expect(hooks).toContain('/T /F');
    expect(hooks).not.toMatch(/taskkill\.exe[^\n]*node\.exe/i);
  });
